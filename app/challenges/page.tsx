...
import { Flag, Search, Download, CheckCircle, X, Shield, Code2, Cpu, Lock, Terminal, HelpCircle, Send, AlertCircle, Zap } from 'lucide-react';
...
export default function ChallengesPage() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      // Try loading live challenges from Supabase
  ...
  return (
    <div className="py-8 sm:py-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-slate-800/80 pb-6">
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 font-mono text-xs font-bold mb-2">
            <Zap className="h-3.5 w-3.5" />
            TARGET MATRIX ACTIVE
          </div>
          <h1 className="text-3xl font-black font-mono text-white tracking-tight">CHALLENGE TERMINAL</h1>
          <p className="text-xs text-slate-400 font-mono mt-1">
            Select a target, download challenge artifacts, and submit flags.
          </p>
        </div>
        ...
        <div className="flex items-center gap-4 font-mono text-xs">
          ...
          <div className="cyber-card rounded-lg px-4 py-2.5 border border-slate-800">
            <span className="text-slate-400 block text-[10px]">ACTIVE TEAM</span>
      ...
      <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4">
        ...
        <div className="relative min-w-[260px]">
          ...
          <input
            ...
            className="w-full rounded-lg bg-slate-950 border border-slate-800 pl-10 pr-4 py-2 text-xs font-mono text-slate-200 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none transition-all"
          />
        </div>
      </div>
            ...
            >
              <div>
                <div className="flex items-center justify-between gap-2 mb-3">
                  <span className={`px-2.5 py-0.5 rounded border text-[10px] font-mono font-bold uppercase tracking-wider ${categoryStyle}`}>
                    {chal.category}
                  </span>
                  
                  <div className="flex items-center gap-2">
                    {isSolved && (
                      <span className="flex items-center gap-1 text-[10px] font-mono font-bold text-emerald-400 bg-emerald-500/20 px-2 py-0.5 rounded border border-emerald-500/30">
              ...
              <div className="mt-6 pt-4 border-t border-slate-800/60 flex items-center justify-between font-mono text-[11px] text-slate-500">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="cyber-card rounded-2xl max-w-2xl w-full p-6 sm:p-8 border border-emerald-500/40 bg-[#090b12] relative shadow-2xl max-h-[90vh] overflow-y-auto space-y-6">
            ...
            <div className="flex items-start justify-between gap-4 border-b border-slate-800 pb-4">
              <div>
                <div className="flex items-center gap-2 mb-2">
                  <span className={`px-2.5 py-0.5 rounded border text-[10px] font-mono font-bold uppercase tracking-wider ${getCategoryStyle(selectedChallenge.category)}`}>
                  ...
                  {solvedIds.has(selectedChallenge.id) && (
                    <span className="font-mono text-xs font-bold text-emerald-400 flex items-center gap-1 bg-emerald-500/20 px-2 py-0.5 rounded border border-emerald-500/30">
                      <CheckCircle className="h-3 w-3" /> SOLVED
                    </span>
                  )}
                </div>
                <h2 className="text-2xl font-black font-mono text-white">{selectedChallenge.title}</h2>
                <p className="text-xs font-mono text-slate-500 mt-0.5">AUTHOR: {selectedChallenge.author}</p>
              </div>
            ...
            <div className="space-y-2">
              <h3 className="font-mono text-xs font-bold text-slate-400 uppercase tracking-wider">CHALLENGE BRIEFING</h3>
            ...
            {selectedChallenge.file_url && (
              <div className="space-y-2">
                <h3 className="font-mono text-xs font-bold text-slate-400 uppercase tracking-wider">ATTACHED ARTIFACT</h3>
                <a
                  href={selectedChallenge.file_url}
                  download
                  className="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg bg-slate-900 border border-emerald-500/40 text-emerald-400 font-mono text-xs font-bold hover:bg-emerald-500/10 hover:border-emerald-400 transition-all"
                >
            ...
            <div className="space-y-3 pt-2">
              <h3 className="font-mono text-xs font-bold text-slate-400 uppercase tracking-wider">SUBMIT FLAG</h3>
              ...
              <form onSubmit={handleFlagSubmit} className="flex gap-2">
                <input
                  ...
                  disabled={submitting}
                  className="flex-1 rounded-lg bg-slate-950 border border-slate-800 px-4 py-3 text-xs font-mono text-slate-100 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none transition-all disabled:opacity-50"
                />
                <button
                  type="submit"
                  disabled={submitting || !flagInput.trim()}
                  className="px-6 py-3 rounded-lg bg-emerald-500 text-slate-950 font-mono font-black hover:bg-emerald-400 hover:shadow-[0_0_15px_rgba(0,255,102,0.4)] transition-all disabled:opacity-50 flex items-center gap-2 whitespace-nowrap"
                >
