...
import { Trophy, Loader2, RefreshCw, Search, Eye, Snowflake, Droplets } from 'lucide-react';
...
export default function LeaderboardPage() {
  ...
  const loadLeaderboard = async () => {
    ...
    try {
      ...
      // Fetch scoreboard settings (frozen flag + frozen timestamp)
      const { data: settingsData } = await supabase
        .from('settings')
        .select('key, value')
        .in('key', ['scoreboard_frozen', 'scoreboard_frozen_at']);

      const settingsMap: Record<string, string> = {};
      (settingsData || []).forEach((s: any) => { settingsMap[s.key] = s.value; });

      const frozen = settingsMap.scoreboard_frozen === 'true';
      const frozenAtStr = settingsMap.scoreboard_frozen_at || null;
      setIsFrozen(frozen);
      setFrozenAt(frozenAtStr);

      // The cutoff time for solves: if frozen, only show solves before freeze time
      const cutoffTime = frozen && frozenAtStr ? frozenAtStr : null;

      // Fetch teams, profiles, solves (respecting freeze cutoff)
      const [teamsRes, profilesRes, solvesRes] = await Promise.all([
        ...
        cutoffTime
          ? supabase.from('solves').select('id, team_id, points, created_at').lte('created_at', cutoffTime)
      ...
      const parsed: LeaderboardEntry[] = teams.map((team: any) => {
        ...
        const teamMems = profiles.filter((p) => p.team_id === team.id);
        const totalPts = teamSolves.reduce((acc: number, s: any) => acc + (s.points || 0), 0);

        let lastTime: string | null = null;
        if (teamSolves.length > 0) {
          const sortedTimes = teamSolves.map((s: any) => new Date(s.created_at).getTime()).sort((a, b) => b - a);
      ...
      parsed.forEach((entry, idx) => { entry.rank = idx + 1; });
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-1">
            {leaderboard.length > 0 ? `${leaderboard.length} teams` : 'No teams registered yet'}
            {isFrozen && frozenAt && (
              <span className="ml-2 text-cyan-500">· Frozen at {new Date(frozenAt).toLocaleString()}</span>
            )}
          </p>
        </div>
      ...
      {isFrozen && (
        <div className="p-3.5 rounded-lg bg-cyan-500/10 border border-cyan-500/20 text-xs flex items-center gap-3 text-cyan-400">
          ...
          <span>
            ...
            {frozenAt ? ` as of ${new Date(frozenAt).toLocaleString()}` : ''}.
            Final standings will be revealed at the end of the event.
          </span>
        </div>
      )}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              <thead className="bg-zinc-900/50 border-b border-zinc-800">
                <tr>
                  <th scope="col" className="py-3 px-4 font-medium text-zinc-500 w-16">Rank</th>
                  <th scope="col" className="py-3 px-4 font-medium text-zinc-500">Team</th>
                  ...
                  <th scope="col" className="py-3 px-4 font-medium text-zinc-500 text-right hidden sm:table-cell">Last Solve</th>
                  ...
                  return (
                    <tr
                      ...
                      className={`transition-colors cursor-pointer ${
                        isCurrentTeam ? 'bg-emerald-500/5 border-l-2 border-l-emerald-500' : 'hover:bg-zinc-800/30'
                      }`}
                    ...
                    >
                      <td className="py-3 px-4">
                        <span className={`inline-flex items-center justify-center w-7 h-7 rounded-md text-xs font-semibold ${
                          entry.rank === 1 ? 'bg-amber-500/15 text-amber-400 border border-amber-500/30'
                          : entry.rank === 2 ? 'bg-zinc-400/15 text-zinc-300 border border-zinc-400/30'
                          : entry.rank === 3 ? 'bg-amber-700/15 text-amber-600 border border-amber-700/30'
                      ...
                      <td className="py-3 px-4 text-center text-zinc-400 text-xs">{entry.members_count}</td>
                      <td className="py-3 px-4 text-center font-medium text-zinc-300">{entry.solves_count}</td>
                      ...
                      <td className="py-3 px-4 text-right text-zinc-500 text-xs hidden sm:table-cell">
                        {entry.last_solve_time
                          ? new Date(entry.last_solve_time).toLocaleString(undefined, { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })
                          : '—'}
                      </td>
                      <td className="py-3 px-4 text-right">
                        <button
                          onClick={(e) => { e.stopPropagation(); handleOpenTeamModal(entry); }}
                          className="p-1 rounded text-zinc-500 hover:text-zinc-200 hover:bg-zinc-800 transition-colors"
                          aria-label={`View ${entry.team_name} details`}
                        >
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between border-b border-zinc-800 pb-3">
              <div>
                ...
                <p className="text-lg font-bold text-amber-400">#{selectedTeamModal.rank} · {selectedTeamModal.total_points} pts</p>
              </div>
              <div className="text-right">
                <p className="text-xs text-zinc-500">Team Stats</p>
                <p className="text-sm font-semibold text-zinc-200">{selectedTeamModal.members_count} members · {selectedTeamModal.solves_count} solves</p>
              </div>
            </div>

            {modalLoading ? (
              <div className="py-8 flex justify-center"><Loader2 className="h-5 w-5 animate-spin text-zinc-600" /></div>
            ) : (
              <>
                <div>
